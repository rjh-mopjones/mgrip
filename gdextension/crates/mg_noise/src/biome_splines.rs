...
pub enum ClimateClass { Frozen, Cold, Temperate, Warm, Hot, Scorching }

impl ClimateClass {
    pub fn from_temperature(temp: f64) -> Self {
        if temp < -20.0 { Self::Frozen }
        else if temp < 3.0 { Self::Cold }
        else if temp < 35.0 { Self::Temperate }
        else if temp < 55.0 { Self::Warm }
        else if temp < 80.0 { Self::Hot }
        else { Self::Scorching }
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum MoistureClass { Arid, Dry, Moderate, Humid, Saturated }

impl MoistureClass {
    pub fn from_humidity(h: f64) -> Self {
        if h < 0.2 { Self::Arid }
        else if h < 0.4 { Self::Dry }
        else if h < 0.6 { Self::Moderate }
        else if h < 0.8 { Self::Humid }
        else { Self::Saturated }
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ElevationClass { Coastal, Lowland, Upland, Highland, Alpine }

impl ElevationClass {
    pub fn from_elevation(above_sea: f64) -> Self {
        if above_sea < 0.04 { Self::Coastal }
        else if above_sea < 0.12 { Self::Lowland }
        else if above_sea < 0.25 { Self::Upland }
        else if above_sea < 0.38 { Self::Highland }
        else { Self::Alpine }
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum TerrainClass { Flat, Rolling, Rugged }

impl TerrainClass {
    /// High erosion = flat; low erosion = rugged peaks preserved.
    pub fn from_erosion(erosion: f64) -> Self {
        if erosion < 0.3 { Self::Rugged }
        else if erosion < 0.7 { Self::Rolling }
        else { Self::Flat }
    }
}
...
impl BiomeSplines {
    ...
    fn compute_elevation(&self, cont: f64, pv: f64, erosion: f64, _tectonic: f64) -> f64 {
        ...
        let peak_height = if is_land { pv.max(0.0) * 0.25 * erosion_damp } else { 0.0 };
        let valley_depth = if is_land { pv.min(0.0).abs() * 0.12 } else { 0.0 };
        cont + peak_height - valley_depth
    }

    fn ocean_biome(&self, elevation: f64, temp: f64, tectonic: f64) -> TileType {
        if temp < -15.0 { return TileType::White; }
        if temp > 80.0 { return TileType::SaltFlat; }
        let depth = self.sea_level - elevation;
        if depth < 0.05 && temp > 20.0 && temp < 35.0 && tectonic > 0.5 { return TileType::CoralReef; }
        if tectonic < 0.2 && depth > 0.3 { return TileType::OceanTrench; }
        if tectonic < 0.3 && depth > 0.1 { return TileType::OceanRidge; }
        if depth < 0.05 { TileType::ShallowSea }
        else if depth < 0.15 { TileType::ContinentalShelf }
        else if depth > 0.25 { TileType::DeepOcean }
        else { TileType::Sea }
    }

    fn adjust_humidity(&self, humidity: f64, elevation: f64) -> f64 {
        let above_sea = (elevation - self.sea_level).max(0.0);
        let rain_shadow = if above_sea > 0.15 { ((above_sea - 0.15) * 2.5).min(0.4) } else { 0.0 };
