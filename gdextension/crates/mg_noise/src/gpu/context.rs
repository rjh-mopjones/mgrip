...
impl GpuNoiseContext {
    ...
    pub fn is_available() -> bool { Self::global().is_some() }
    ...
    pub fn generate_layers(
        ...
        let continentalness = self.dispatch_independent(
            &self.pipelines.continentalness, &self.pipelines.continentalness_layout,
            NoiseParams {
                seed, width: tile_w as u32, height: tile_h as u32,
                octaves: 16 + detail_level,
                frequency: 1.0, persistence: 0.59, lacunarity: 2.0,
                scale: scale as f32,
                world_x: world_x as f32, world_y: world_y as f32,
                world_height: world_height as f32, _padding: 0.0,
            },
            &cont_perm, tile_w, tile_h,
        );

        let light_level = self.dispatch_independent(
            &self.pipelines.light_level, &self.pipelines.light_level_layout,
            NoiseParams {
                seed: seed.wrapping_add(4),
                width: tile_w as u32, height: tile_h as u32,
                octaves: 3 + detail_level,
                frequency: 1.0, persistence: 0.5, lacunarity: 2.0,
                scale: scale as f32,
                world_x: world_x as f32, world_y: world_y as f32,
                world_height: world_height as f32, _padding: 0.0,
            },
            &light_perm, tile_w, tile_h,
        );

        let rock_hardness = self.dispatch_independent(
            &self.pipelines.rock_hardness, &self.pipelines.rock_hardness_layout,
            NoiseParams {
                seed: seed.wrapping_add(3),
                width: tile_w as u32, height: tile_h as u32,
                octaves: 3 + detail_level,
                frequency: 1.0, persistence: 0.6, lacunarity: 2.0,
                scale: scale as f32,
                world_x: world_x as f32, world_y: world_y as f32,
                world_height: 0.0, _padding: 0.0,
            },
            &rock_perm, tile_w, tile_h,
        );

        let peaks_valleys = self.dispatch_independent(
            &self.pipelines.peaks_valleys, &self.pipelines.peaks_valleys_layout,
            NoiseParams {
            ...
            &peak_perm, tile_w, tile_h,
        );

        let humidity = self.dispatch_dependent(
            &self.pipelines.humidity, &self.pipelines.humidity_layout,
            NoiseParams {
            ...
            &continentalness, &humi_perm, tile_w, tile_h,
        );

        GpuNoiseResult { continentalness, peaks_valleys, humidity, light_level, rock_hardness }
    }
    ...
    fn dispatch_dependent(
        ...
        let cont_buf = self.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: None, contents: bytemuck::cast_slice(cont),
            usage: wgpu::BufferUsages::STORAGE,
        });
        let out_buf = self.device.create_buffer(&wgpu::BufferDescriptor {
            label: None, size: buf_size,
        ...
        let bind_group = self.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: params_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: perm.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 2, resource: cont_buf.as_entire_binding() },
